...
const Header: React.FC<HeaderProps> = ({ cartItemCount, onCartClick, onScrollToTop, isCartAnimating }) => {
  return (
    <header className="bg-brand-cream/90 backdrop-blur-sm sticky top-0 z-40 shadow-sm border-b border-brand-red/10 pt-[env(safe-area-inset-top)]">
      <div className="container mx-auto px-4 h-[92px] md:h-[129px] flex justify-between items-center">
        <div className="flex items-center">
          <button onClick={onScrollToTop} className="group" aria-label="Torna in cima">
            <Logo className="h-[74px] md:h-[95px] w-auto"/>
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <ShoppingCartIcon className="h-[37px] w-[37px] md:h-[55px] w-[55px]" />
            {cartItemCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-brand-red text-white text-xs md:text-sm font-bold rounded-full h-[23px] w-[23px] md:h-[37px] w-[37px] flex items-center justify-center animate-pulse">
                {cartItemCount}
              </span>
            )}
          </button>
